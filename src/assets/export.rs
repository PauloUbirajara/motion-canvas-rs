...
/// Starts a background FFmpeg process to encode raw RGBA frames into a video.
///
/// This function returns a piped `stdin` handle. The caller is responsible for
/// writing raw RGBA byte arrays into this pipe.
#[cfg(feature = "export")]
pub fn start_ffmpeg(
    title: &str,
    width: u32,
...
/// Merges multiple audio event streams into the exported video file.
///
/// `merge_audio` uses FFmpeg's `amix` and `adelay` filters to precisely align
/// audio clips with the video timeline. It works on the temporary video
/// generated by `start_ffmpeg` and produces the final output file.
#[cfg(feature = "export")]
